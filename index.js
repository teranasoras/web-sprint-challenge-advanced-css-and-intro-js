...
function getArtistByIndex(arr, index) {
    return("The artist at " + (arr[index]["id"]) + " is " + (arr[index]["name"]));
  }
...
function get20s(arr){
  for(let i = 0; i < arr.length; i++){
    checkarray.push((arr[i]["years"].split(' ')));
    birthyear.push(checkarray[i][0]);
    deathyear.push(checkarray[i][2]);
    if(birthyear[i] > 1899){
      if(deathyear[i] < 2000){
      livedin20s.push(arr[i]["name"]);
    }
  }
}
return(livedin20s)
...
function addArtist(arr, myid, myname, myyears, mygenre, mynationality, mybio, mypaintings){
  arr.push({
    id: myid,
    name: myname,
    years: myyears,
...
console.log(addArtist(artists, 20, "Tyler Maynard", "1999 - 2020", "web development", "American", "Tyler maynard has never painted anything in his life. He in fact accidentally destroyed a painting when he was you which leads to his low number of paintings", "-1"))
